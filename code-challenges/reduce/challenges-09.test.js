...
const returnNames = (arr) => {
  // Solution code here...
  let resultsArr = [];
  let results = arr.reduce((acc,val)=>{
      resultsArr.push(val.name)
    return acc;
  },[])
  return resultsArr;
};
...
const reversedString = (str) => {
  // Solution code here...
  let split = str.split('');
  let reveresString = split.reduce((acc,val)=>{
      return val + acc;
  },'')
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let numChildren = arr.reduce((acc,val)=>{
...
const calculateAverage = (arr) => {
  // Solution code here...
  let newArray =arr.reduce((acc,value,index)=>{
  acc.count = index+1;
  acc.sum += value
  return acc;
  },{ count: 0, sum: 0 })
  return newArray.sum/newArray.count;
};
