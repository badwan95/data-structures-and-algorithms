...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(value =>{
    ...
    let firstSlice = value.slice(firstIndex+1);
    let secondIndex = firstSlice.indexOf(' ');
    let secondSlice = firstSlice.slice(secondIndex+1);
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  recipe.ingredients.forEach(value =>{
    let splitting = value.split(' ');
    splitting.splice(0,2);
    let newString = splitting.join(' ');
...
const extractVowels = (str) => {
  ...
  vowelArray.sort((a,b) =>{
    let firstVowel = a.toLowerCase();
    let secondVowel = b.toLowerCase();
  ...
  let firstElement = newArray.join('');
  let secondElement = vowelArray.join('');
