...
const count = (target, input) => {
  ...
  let newArray = input.map(value=>{
      let arrayAnswer = value.map(value=>{
          if (target === value){
            count++;
          }
      })
...
const totalSum = (input) => {
  // Solution code here...
  let resultOfInternal = input.map(value=>{
      let sumOfInternalArr = value.reduce((acc,val)=>{
        return acc+val;
      },0)
      return sumOfInternalArr;
  })
  let finalSum = resultOfInternal.reduce((acc,val)=>{
      return acc+val;
  },0)
  return finalSum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(value=>{
      let h = [];
      value.forEach(val=>{
        if(!isNaN(val) === true && val%5 === 0){
            let jal = Math.pow(2,val);
            h.push(jal);
        }
      })
      ia.push(h);
...
//   input.forEach(value=>{
//       value.forEach(val=>{
//           if(!isNaN(val) && value % 5 === 0){
//               smallArray.push(Math.pow(val));
//           }
//       })
//   })
};
...
let findShortest = (data) => {
  // Solution code here...
  let heights = data.map(value=>{
      return value.height;
  })
  let j = heights.sort((a,b)=>{return b-a});
  let shortest = j[j.length-1];
  let name;
  data.forEach(value=>{
      if (shortest === value.height){
          name = value.name;
      }
  })
  return name;
