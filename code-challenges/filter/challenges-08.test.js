...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let newArr = arr.filter(value=>{
    let vowelValidator = /[aeiou]/g;
    let answer = vowelValidator.test(value);
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let newArr = arr.filter(arrValue=>{
    let answer;
    forbiddenValues.forEach(forbiddenValue=>{
        if (arrValue === forbiddenValue){
            return answer=true;
        }
    })
    if (!answer){return arrValue};
  })
  return newArr;
};
...
console.log(`array result ${newArr}`)
return answerArr;
};
